...
export class TextWrap implements WrapStyle {
	...
	wrap(text: string, indents: string): WrapResult {
		...
		mainLoop: for (let i = 0, vLen = 0, vLen0 = 0, start = 0, marker = 0, c: string; i < length; ++i) {
			c = text[i];
			vLen += c === '\t' ? tabLength : 1;
			...
			searchBreakLine: do {
				...
				searchWhiteSpace: do {
					if (vLen > this.wrapOn &&
							...
							&& isNotBreakable // [\S\x0A]: All non-breakable characters ('\n' already filtered)
					) break searchWhiteSpace;
					
					else if (isNotBreakable) continue mainLoop;
